...
<form action="<?php echo e(route('admin.berita.update', $berita->id)); ?>" method="POST" enctype="multipart/form-data" 
      x-data="beritaEditor(`<?php echo addslashes($berita->isi); ?>`, `<?php echo e($berita->thumbnail ? asset('uploads/berita/' . $berita->thumbnail) : ''); ?>`)">
    <?php echo csrf_field(); ?>
    <?php echo method_field('PUT'); ?> <input type="hidden" name="status" x-model="status">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div class="lg:col-span-8 space-y-8">
            ...
            <div>
                ...
                <textarea name="judul" rows="2" required class="w-full bg-white border border-gray-100 rounded-3xl p-6 text-3xl font-extrabold text-brand-dark placeholder-gray-300 outline-none focus:ring-2 focus:ring-brand-blue/10 resize-none shadow-sm"><?php echo e(old('judul', $berita->judul)); ?></textarea>
            </div>
            ...
            <div class="grid grid-cols-2 gap-6">
                <div>
                    ...
                    <div class="relative">
                        <select name="kategori" required class="w-full bg-gray-50/80 border border-gray-100 rounded-2xl px-5 py-4 text-[14px] font-bold text-brand-dark outline-none focus:ring-2 focus:ring-brand-blue/10 appearance-none shadow-sm">
                            <option value="Akademik" <?php echo e($berita->kategori == 'Akademik' ? 'selected' : ''); ?>>Akademik</option>
                            <option value="Beasiswa" <?php echo e($berita->kategori == 'Beasiswa' ? 'selected' : ''); ?>>Beasiswa</option>
                            <option value="Kegiatan" <?php echo e($berita->kategori == 'Kegiatan' ? 'selected' : ''); ?>>Kegiatan</option>
                        </select>
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="datetime-local" name="tanggal_publish" value="<?php echo e($berita->tanggal_publish ? \Carbon\Carbon::parse($berita->tanggal_publish)->format('Y-m-d\TH:i') : ''); ?>" class="w-full bg-gray-50/80 border border-gray-100 rounded-2xl px-5 py-4 text-[14px] font-bold text-brand-dark outline-none focus:ring-2 focus:ring-brand-blue/10 shadow-sm cursor-pointer">
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="ringkasan" rows="3" required class="w-full bg-gray-50/80 border border-gray-100 rounded-3xl p-5 text-[14px] font-medium text-brand-dark placeholder-gray-400 outline-none focus:ring-2 focus:ring-brand-blue/10 resize-none shadow-sm"><?php echo e(old('ringkasan', $berita->ringkasan)); ?></textarea>
            </div>
            ...
            <div>
                ...
                <div class="bg-white border border-gray-100 rounded-[2rem] shadow-sm overflow-hidden flex flex-col min-h-[400px]">
                    ...
                    <textarea x-ref="editor" x-model="konten" name="konten" required class="w-full flex-1 p-8 outline-none resize-none text-[15px] font-medium text-brand-dark placeholder-gray-400 leading-relaxed" placeholder="Mulai menulis di sini...">
                        <?php echo e(old('konten', $berita->isi)); ?>
        ...
        <div class="lg:col-span-4 space-y-6 sticky top-32">
            ...
            <div class="bg-gray-50/50 border border-gray-100 p-6 rounded-[2rem] shadow-sm">
                ...
                <div class="mb-5">
                    ...
                    <input type="text" value="/berita/<?php echo e($berita->slug); ?>" class="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl outline-none focus:border-brand-blue text-[12px] font-mono text-gray-500 shadow-sm" readonly>
                </div>
            </div>
...
document.addEventListener('alpine:init', () => {
    ...
    Alpine.data('beritaEditor', (initialKonten, initialImage) => ({
        status: '<?php echo e($berita->status ?? "Published"); ?>',
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\Database\spmb-adzkia\resources\views/admin/berita-edit.blade.php ENDPATH**/ ?>