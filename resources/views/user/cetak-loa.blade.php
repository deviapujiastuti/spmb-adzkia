...
<head>
    ...
    <title>Letter of Acceptance - {{ $pendaftar->nama_lengkap }}</title>
...
<body class="text-black">
    ...
    <div class="text-center mt-8 mb-4 no-print flex justify-center gap-4">
        <a href="{{ route('dashboard.user') }}" class="px-6 py-2.5 bg-gray-500 text-white rounded-lg font-sans font-bold hover:bg-gray-600 transition-colors">Kembali</a>
    ...
    <div class="a4-container">
        ...
        <div class="flex items-center border-b-4 border-black pb-4 mb-8">
            <img src="{{ asset('images/logo-adzkia.png') }}" alt="Logo Universitas" class="w-24 h-auto mr-6">
            ...
            <div class="w-24"></div> {{-- Spacer penyeimbang --}}
        </div>

        {{-- JUDUL SURAT --}}
        <div class="text-center mb-8">
            <h3 class="text-[18px] font-bold underline mb-1">SURAT KETERANGAN LULUS SELEKSI</h3>
            <p class="text-[14px]">Nomor: {{ date('Y') }}/SPMB/{{ substr($pendaftar->no_pendaftaran, -4) }}</p>
        </div>

        {{-- ISI SURAT --}}
        <div class="text-[15px] leading-relaxed text-justify mb-8 space-y-4">
            <p>Berdasarkan hasil evaluasi dan penilaian dari Panitia Penerimaan Mahasiswa Baru Universitas Adzkia Tahun Akademik {{ date('Y') }}/{{ date('Y')+1 }}, dengan ini Rektor Universitas Adzkia menerangkan bahwa:</p>
            ...
            <div class="pl-8 mb-4">
                <table class="w-full">
                    <tr>
                        ...
                        <td class="w-4">:</td>
                        <td class="font-bold uppercase">{{ $pendaftar->nama_lengkap }}</td>
                    </tr>
                    <tr>
                        <td class="py-1">Nomor Registrasi</td>
                        <td>:</td>
                        <td class="font-bold">{{ $pendaftar->no_pendaftaran }}</td>
                    </tr>
                    <tr>
            ...
            <div class="text-center my-6 p-4 border-2 border-black rounded bg-gray-50">
                ...
                <h3 class="text-[20px] font-bold uppercase">
                    {{ $pendaftar->status_kelulusan == 'Lulus Pilihan 1' ? $pendaftar->pilihan_jurusan_1 : $pendaftar->pilihan_jurusan_2 }}
                </h3>
            </div>
        ...
        <div class="flex justify-end mt-12">
            <div class="text-center w-64">
                <p class="mb-1">Padang, {{ \Carbon\Carbon::now()->translatedFormat('d F Y') }}</p>
