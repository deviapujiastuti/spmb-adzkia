...
<div x-data="faqManager()">
    ...
    {{-- LIST FAQ DINAMIS DARI DATABASE --}}
    <div class="space-y-4 max-w-4xl mb-10">
        ...
        <div class="bg-white rounded-2xl shadow-sm border overflow-hidden transition-all duration-300"
             :class="activeFaq === {{ $faq->id }} ? 'border-l-4 border-l-brand-dark border-t-gray-100 border-r-gray-100 border-b-gray-100' : 'border-gray-100 border-l-transparent hover:border-l-gray-300'">
            <div class="p-6 cursor-pointer flex items-start gap-4" @click="activeFaq = activeFaq === {{ $faq->id }} ? null : {{ $faq->id }}">
                ...
                <div class="flex-1">
                    <div class="flex justify-between items-center mb-3">
                        ...
                        <span class="px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest 
                            {{ $faq->kategori == 'Dashboard Utama' ? 'bg-blue-50 text-brand-blue' : 'bg-indigo-50 text-indigo-600' }}">
                            {{ $faq->kategori }}
                        </span>
                        
                        <div class="flex items-center gap-3 text-gray-400">
                            {{-- TOMBOL EDIT --}}
                            <button @click.stop='bukaModalEdit({{ $faq->id }}, {{ json_encode($faq->pertanyaan) }}, {{ json_encode($faq->jawaban) }}, "{{ $faq->kategori }}")' class="hover:text-brand-dark transition-colors" title="Edit FAQ">
                                <i data-feather="edit-2" class="w-4 h-4"></i>
                            </button>
                            
                            {{-- TOMBOL HAPUS --}}
                            <form action="/admin/faq/{{ $faq->id }}" method="POST" class="inline" @click.stop onsubmit="return confirm('Apakah Anda yakin ingin menghapus FAQ ini?');">
                                @csrf
                                @method('DELETE')
                            ...
                            <i data-feather="chevron-down" class="w-5 h-5 transition-transform duration-300 ml-2" :class="activeFaq === {{ $faq->id }} ? 'rotate-180' : ''"></i>
                        </div>
                    </div>
                    <h3 class="text-[16px] font-extrabold text-brand-dark leading-snug pr-8">{{ $faq->pertanyaan }}</h3>
        ...
        <div x-show="modalOpen" class="fixed inset-0 z-[999] flex items-center justify-center p-4" x-cloak>
            ...
            <div x-show="modalOpen" 
                ...
                <form :action="formAction" method="POST">
                    @csrf
                    {{-- Alpine mendeteksi jika ini Edit, maka method spoofing PUT akan digunakan --}}
                    <template x-if="formMethod === 'PUT'">
                        <input type="hidden" name="_method" value="PUT">
                    </template>
                    
                    {{-- Input Hidden untuk Kategori yang dipilih lewat Button --}}
